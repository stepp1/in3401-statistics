...
def open_global_ts(path):
    ...
    ini, end = parse(ts.columns[3]), parse(ts.columns[-1]); delta = end - ini
...
def ts_since_two_per_country(df):
    ...
    countries = [df[[country] >= 2][country].reset_index()[[country]] for country in df.columns]
...
def compute_tasa_contagio(pais, inicio, fin, return_params=False):
    ...
    confirmados_horizonte_ini = ts_confirmed[ts_confirmed.index == inicio][pais].values
    confirmados_horizonte_end = ts_confirmed[ts_confirmed.index == fin][pais].values
    cofirmados_horizonte = confirmados_horizonte_end - confirmados_horizonte_ini

    recuperados_horizonte_ini = ts_recovered[ts_recovered.index == inicio][pais].values
    recuperados_horizonte_end = ts_recovered[ts_recovered.index == fin][pais].values
    ...
    poblacion_total = info_countries[info_countries['Country'] == pais]['Population (2020)'].values
    
    casos_activos_horizonte = cofirmados_horizonte - muertos_horizonte - recuperados_horizonte  
    pop_riesgo_horizonte = poblacion_total - muertos_horizonte - recuperados_horizonte - casos_activos_horizonte
